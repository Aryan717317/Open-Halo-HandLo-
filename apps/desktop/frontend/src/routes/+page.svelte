<script lang="ts">
  ...
  import { startDiscovery, pickFile, sendFile } from '$lib/transfer/tauribridge';
  ...
  function handleFrame(result: any) {
    ...
    gestureStore.set({ gesture, landmarks, confidence: classifier.confidence, handVisible: landmarks.length > 0 });
  ...
  async function handleGrab(palmLandmark: any) {
    ...
    if (!path) { screen = 'ready'; return; }

    selectedFile = path;
    selectedFileName = path.split('/').pop() ?? path.split('\\').pop() ?? 'file';
    screen = 'selecting';
  ...
  async function handleSend() {
    ...
    try {
      ...
      statusMessage = 'Sent!';
      fileOrb?.complete();
      setTimeout(() => { screen = 'ready'; selectedFile = null; }, 2000);
    } catch (e) {
  ...
  function selectPeer(id: string) {
    selectedPeerId = id;
    screen = 'ready';
    statusMessage = 'Peer selected — close fist to grab a file';
  }
</script>
  ...
  <div class="ui">
    ...
    <div class="panel connect-panel">
      ...
      <ul class="peer-list">
        {#each $peers as peer}
        <li>
          <button class="peer-btn" on:click={() => selectPeer(peer.id)}>
...
<style>
  ...
  :global(body) { background: #000; overflow: hidden; font-family: 'SF Pro Display', system-ui, sans-serif; }
  ...
  .status-dot.active { background: #39ff14; box-shadow: 0 0 8px #39ff14; }
  ...
  .peer-meta { font-size: 11px; color: rgba(255,255,255,0.4); }
  ...
  .hint-text { font-size: 14px; color: rgba(255,255,255,0.8); }
  ...
  .progress-label { font-size: 13px; color: #fff; margin-bottom: 8px; }
  .progress-track {
    height: 4px;
  ...
  .progress-pct { font-size: 11px; color: rgba(255,255,255,0.5); margin-top: 5px; text-align: right; }
</style>
